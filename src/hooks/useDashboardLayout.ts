...
export function useDashboardLayout() {
  ...
  const loadLayout = async () => {
    ...
    try {
      ...
      if (hasValidLayout) {
        // Sanitize: keep only known widget IDs from the catalog
        const knownIds = new Set(Object.keys(WIDGET_CATALOG));
        ...
        // Filter layout items to only include valid widgets
        let filteredLayout = (savedLayout.layout || []).filter((item) => knownIds.has(item.i));

        // Ensure defaults are present if missing (optional, mostly for core widgets)
        for (const l of DEFAULT_LAYOUT) {
          if (!filteredLayout.some(item => item.i === l.i)) {
            filteredLayout.push(l);
          }
        }

        // Filter widget configs
        ...
        for (const w of DEFAULT_WIDGETS) {
          if (!filteredWidgets.some(item => item.id === w.id)) {
