...
export const MonthlyReport = ({ trades }: MonthlyReportProps) => {
  ...
  // Use net PnL (after fees) for all calculations
  const totalPnl = calculateTotalPnL(monthTrades, { includeFees: true });
  const winningTrades = monthTrades.filter(t => calculateTotalPnL([t], { includeFees: true }) > 0);
  const losingTrades = monthTrades.filter(t => calculateTotalPnL([t], { includeFees: true }) < 0);
  ...
  const bestTrade = monthTrades.length > 0
    ? monthTrades.reduce((best, t) => {
      const tPnl = calculateTotalPnL([t], { includeFees: true });
      const bestPnl = calculateTotalPnL([best], { includeFees: true });
      return tPnl > bestPnl ? t : best;
    })
    : null;

  const worstTrade = monthTrades.length > 0
    ? monthTrades.reduce((worst, t) => {
      const tPnl = calculateTotalPnL([t], { includeFees: true });
      const worstPnl = calculateTotalPnL([worst], { includeFees: true });
      return tPnl < worstPnl ? t : worst;
    })
    : null;

  // Asset performance (use net PnL)
  const assetPnl: Record<string, { pnl: number; trades: number }> = {};
  monthTrades.forEach(trade => {
    if (!assetPnl[trade.symbol]) {
      assetPnl[trade.symbol] = { pnl: 0, trades: 0 };
    }
    assetPnl[trade.symbol].pnl += calculateTotalPnL([trade], { includeFees: true });
  ...
  // Trading days calculation respecting user setting
  const { tradingDays } = calculateTradingDays(monthTrades, tradingDaysMode);

  // Daily P&L (use closed_at for day grouping, fallback to trade_date)
  const dailyPnl: Record<string, number> = {};
  monthTrades.forEach(trade => {
    const dateToUse = trade.closed_at || trade.trade_date;
    const day = new Date(dateToUse).toDateString();
    dailyPnl[day] = (dailyPnl[day] || 0) + calculateTotalPnL([trade], { includeFees: true });
  });
      ...
      {monthTrades.length === 0 ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <PremiumCard className={`p-4 ${totalPnl > 0
            ...
            <PremiumCard className="p-4 bg-muted/20">
              <div className="flex items-center gap-2 mb-2">
            ...
            <PremiumCard className="p-4 bg-muted/20">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-muted-foreground">Trading Days ({tradingDaysMode === 'calendar' ? 'Calendar' : 'Active'})</span>
              </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-bold text-neon-green">
                        +${calculateTotalPnL([bestTrade], { includeFees: true }).toFixed(2)}
                      </span>
                    </div>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-bold text-neon-red">
                        ${calculateTotalPnL([worstTrade], { includeFees: true }).toFixed(2)}
                      </span>
                    </div>
                  </div>
                )}
              ...
              <div className="space-y-2">
                {topAssets.map(([symbol, data], idx) => (
                  <div key={symbol} className="flex items-center justify-between p-2 rounded bg-card">
                    ...
                    <div className={`text-lg font-bold ${data.pnl > 0 ? 'text-neon-green' : 'text-neon-red'
                      }`}>
          ...
          {/* Monthly Summary */}
          <PremiumCard className="p-4 bg-gradient-to-br from-primary/10 to-transparent border-primary/30">
